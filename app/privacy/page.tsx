...
const content = {
  ko: {
    back: '← 홈으로',
    title: '개인정보처리방침',
    lastUpdated: '최종 수정일: 2026년 7월 13일',
    ...
    s1Items: [
      ...
      { label: '쿠키 및 광고 데이터', desc: ': 사용자가 쿠키 배너에서 동의한 경우에만 Google Analytics 및 Google AdSense를 통해 처리될 수 있습니다.' },
    ],
    s2Title: '2. 개인정보 보유 및 이용 기간',
    s2Body: '생년월일은 운세 생성 후 즉시 파기되며 별도로 저장되지 않습니다. 접속 로그는 Cloudflare의 적용 정책과 서비스 설정에 따라 필요한 기간 동안 처리되며, 서비스 안정성 및 보안 목적이 끝난 뒤 삭제됩니다.',
    ...
    s5Body: 'Google Analytics와 Google AdSense는 이용자가 쿠키 배너에서 동의한 경우에만 로드됩니다. 거부하면 분석 및 광고 스크립트가 로드되지 않으며 운세 기능은 그대로 이용할 수 있습니다.',
  ...
  en: {
    ...
    s1Items: [
      ...
      { label: 'Cookies and ad data', desc: ': Processed through Google Analytics and Google AdSense only after consent is given in the cookie banner.' },
    ],
    s2Title: '2. Retention Period',
    s2Body: 'Date of birth is discarded after fortune generation and is not stored separately. Access logs are processed for the period required by the applicable Cloudflare policy and service configuration, then deleted when they are no longer needed for security and service stability.',
    ...
    s5Body: 'Google Analytics and Google AdSense load only after you consent through the cookie banner. If you decline, analytics and advertising scripts are not loaded and the fortune features remain available.',
  ...
  zh: {
    ...
    s1Items: [
      ...
      { label: 'Cookie及广告数据', desc: '：仅在用户通过Cookie横幅同意后，才会由Google Analytics和Google AdSense处理。' },
    ],
    s2Title: '2. 个人信息保留期限',
    s2Body: '出生日期在生成运势后即被丢弃，不会单独存储。访问日志将根据适用的Cloudflare政策和服务设置，在维护安全及服务稳定所需的期限内处理，目的完成后删除。',
    ...
    s5Title: '5. Cookie使用',
    s5Body: '只有在您通过Cookie横幅同意后，Google Analytics和Google AdSense才会加载。拒绝后不会加载分析及广告脚本，运势功能仍可正常使用。',
  ...
  ja: {
    ...
    s1Items: [
      ...
      { label: 'Cookieおよび広告データ', desc: '：Cookieバナーで同意した場合に限り、Google AnalyticsおよびGoogle AdSenseによって処理される場合があります。' },
    ],
    s2Title: '2. 個人情報の保持期間',
    s2Body: '生年月日は運勢生成後に破棄され、別途保存されません。アクセスログは、適用されるCloudflareのポリシーとサービス設定に基づき、セキュリティとサービス安定性に必要な期間処理され、目的終了後に削除されます。',
    ...
    s5Title: '5. Cookieの使用',
    s5Body: 'Google AnalyticsとGoogle AdSenseは、Cookieバナーで同意した場合にのみ読み込まれます。拒否した場合、分析・広告スクリプトは読み込まれず、運勢機能は引き続き利用できます。',
