...
export async function POST(request: NextRequest) {
  try {
    ...
    const genderNoteKo = isFemale ? '여성 아이돌 기준으로 분석합니다.' : '남성 아이돌 기준으로 분석합니다.';
    const genderNoteEn = isFemale ? 'Analyze based on female idol standards.' : 'Analyze based on male idol standards.';

    const prompt = isKorean
      ? `당신은 K-팝 기획사 캐스팅 디렉터입니다. ${genderNoteKo} 이 사람의 얼굴을 5개 항목으로 세부 채점하여 4대 기획사 적합도를 분석해주세요.
...

{
  ...
  "similarIdol": "이름(그룹) 스타일"
      ...
      : `You are a K-pop talent scout. ${genderNoteEn} Score this person's face across 5 dimensions to determine which of the 4 major agencies (SM, JYP, YG, HYBE) they match best.
