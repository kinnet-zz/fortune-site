...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = isKorean
      ? `당신은 K-팝 기획사 캐스팅 디렉터입니다. 이 사람의 얼굴을 5개 항목으로 세부 채점하여 4대 기획사 적합도를 분析해주세요.

[중요: 얼굴 감지 먼저]
사람 얼굴이 전혀 보이지 않으면 {"error": "이유"} 만 반환하세요.
- 동물, 사물, 풍경, 음식, 그림, 만화, 캐릭터 → 즉시 error 반환
- 얼굴이 가려지거나 너무 작아 이목구비 불분명 → error 반환
- 여러 명이 있으면 → 가장 크고 선명하게 보이는 얼굴 1명을 자동 선택하여 분析
...
- Multiple people → automatically select the largest and clearest face for analysis
